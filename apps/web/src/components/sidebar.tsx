		...
		<aside className="w-64 min-h-screen bg-orange-50 dark:bg-orange-500 drop-shadow-lg flex flex-col">
			...
			<nav className="flex flex-col flex-grow">
				{navs.map((nav) => (
					<Link
						href={nav.href}
						key={nav.label}
						className="flex group gap-x-3 rounded-md px-4 py-3 text-md font-semibold leading-6 awf items-center"
					>
						...
						<span className="overflow-hidden text-gray-800 group-hover:text-gray-950 dark:text-gray-100 dark:group-hover:text-gray-200">
							{nav.label}
						</span>
					</Link>
				))}
			...
			<div className="flex items-center space-x-2 p-4">
				<Switch
					...
					className=""
				/>
				<Label htmlFor="dark-mode" className="text-gray-950 dark:text-gray-50">
					Dark Mode
				</Label>
			</div>
		</aside>
